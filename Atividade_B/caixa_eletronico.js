...
//Entrada
let valor = Number(question("Digite o valor da quantia solicitada: "))
...
for (let i = 0; i < notas.length; i++) {
    quant[i] = Math.floor(valorrest / notas[i]);
    valorrest -= quant[i] * notas[i];
  }
  let resultado = '';
  for (let i = 0; i < notas.length; i++) {
    if (quant[i] > 0) {
      resultado += quant[i] + ' nota(s) de R$ ' + notas[i] + ', ';
    }
  }
